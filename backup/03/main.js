...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (scrollHeroSection && heroCanvas) {
    ...
    function handleHeroScrub() {
      ...
      // Hold Scroll: interaction finishes at 82%, remaining 18% holds final state
  ...
  function handleOverviewScroll() {
    ...
    // Hold Scroll: interaction finishes at 82%, remaining 18% holds final step
    const HOLD_START = 0.82;
    let progress = Math.min(1, rawProgress / HOLD_START);
    ...
    function handleHeroParallax() {
      ...
      if (distToHero > 0) {
        ...
        if (relScroll <= heroHeight) {
          const HOLD_DIST = heroHeight * 0.20;
          let exitP = 0;
          if (relScroll > HOLD_DIST) {
            exitP = (relScroll - HOLD_DIST) / (heroHeight - HOLD_DIST);
          }

          if (heroText) {
            heroText.style.transform = `translateY(${exitP * heroHeight * 0.25}px)`;
            heroText.style.opacity = Math.max(0, 1 - exitP * 1.7);
          }
          if (heroVisual) {
            heroVisual.style.transform = `translateY(${exitP * heroHeight * 0.12}px) scale(${1 - exitP * 0.06})`;
            heroVisual.style.opacity = Math.max(0, 1 - exitP * 1.4);
          }
          if (scrollExplore) {
            scrollExplore.style.opacity = Math.max(0, 1 - exitP * 2.8);
          }
        } else {
